...
ALU_Ops decode (const ALU_Type data) {
  if (data.instr.lsli.op == ALU_LSLI_OP) {
    // 315: insert code here to print lsls instruction: COMPLETE
    if (opts.instrs) { 
    ...
    // 315: insert code here to print subs instruction: COMPLETE
    if (opts.instrs) { 
...
DP_Ops decode (const DP_Type data) {
  if (data.instr.DP_Instr.op == DP_CMP) {
    // 315: insert code here to print cmp instruction: COMPLETE
    if (opts.instrs) { 
...
SP_Ops decode (const SP_Type data) {
  if (data.instr.mov.op == 2) {
    ...
    // Here you'll need to SP_ADD similar to above: NEED TO CHECK IF RIGHT
    if (opts.instrs) { 
      cout << "add ";
      if (data.instr.add.d) {
        // These two cases handle stack pointer printing
        if (data.instr.add.rd == 5) {
          ...
          cout << "sp, r" << setbase(10) << (8+data.instr.add.rd) << ", sp" << endl;
        }
        // this case is for registers greater than r7 that aren't sp
        else {
